...
use ndarray::prelude::*;
use crate::reaction::{json_data, thermo::{ThermoInterp, ThermoProp} };
...
impl Gas {
    ...
    /// ```
    pub fn TPX(&mut self, temp: f64, press: f64, mol_frac: &str) {
        self.thermo_prop.T = temp;
        self.thermo_prop.P = press;
        self.X(mol_frac);
    }
    ...
    /// ```
    pub fn X(&mut self, mol_frac: &str) {
    ...
    fn update_prop(&mut self) {
        ...
        for (i, thermo_interp) in self.thermo_interp.iter().enumerate() {
            if self.T() < thermo_interp.Tmid() {
                cp_h_s = 
                    ThermoInterp::calc_thermo_properties( thermo_interp.coeffs_low(), self.T() );
            } else {
                cp_h_s = 
        ...
        self.thermo_prop.mu = (1.458e-6)*(self.T()*self.T()*self.T()/(self.T()+110.4)).sqrt();
    }
